🏥 Healthcare Analytics Dashboard

📌 Project Overview

This Healthcare Analytics Dashboard is developed using Power BI to transform raw healthcare data into meaningful business insights. The dashboard helps analyze patient trends, hospital performance, revenue metrics, and operational KPIs through interactive visualizations.

The project focuses on improving healthcare decision-making using data-driven insights and professional dashboard design techniques.

📊 Key Features
Patient data analysis
Revenue and operational KPI tracking
Interactive filters and slicers
Dynamic charts and visual reports
Professional healthcare dashboard design
Data-driven business insights


📈 Business Insights

Identified that Cigna generated the highest revenue contribution, reaching approximately 287M, making it the top-performing insurance provider in the dataset.
Discovered that the abnormal test percentage was around 34%, highlighting a significant portion of patients requiring further medical attention.
Analyzed monthly revenue trends and found that revenue peaked during July and August, while February recorded the lowest performance.
Evaluated doctor performance based on patient volume, where Michael Smith emerged as the leading doctor with the highest number of patients handled.
Observed that the gender distribution among patients was nearly balanced, indicating an equal representation across genders.

🛠 Tools & Technologies Used

Power BI
Power Query
DAX
Data Modeling
Excel / CSV Dataset

📷 Dashboard Preview
(https://github.com/punitujjainwal/Healthcare-analysis-dashboard/blob/main/Screenshot%202026-05-21%20021903.png)

🚀 Project Highlights

Built an interactive healthcare analytics dashboard
Applied data cleaning and transformation using Power Query
Used DAX measures for KPI calculations
Designed professional and user-friendly visualizations
Focused on real-world healthcare business analysis

👨‍💻 Author

Punit kumar
Aspiring Data Analyst | Power BI Enthusiast | Excel & SQL Learner
